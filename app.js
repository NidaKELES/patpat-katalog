...
function buildPrintArea() {
  ...
  // Kullanıcı hangi filtredeyse onu yazdır (Tümü ise hepsi)
  let list = allProducts.slice();

  if (currentGroup !== "ALL") {
    list = list.filter(p => (p.group || "").trim() === currentGroup);
  }
  if (currentSub) {
    list = list.filter(p => (p.subcategory || "").trim() === currentSub);
  }
  if (searchText) {
    ...
    list = list.filter(p => {
      ...
      const group = (p.group || "").toLowerCase();
      const sub = (p.subcategory || "").toLowerCase();
      return name.includes(q) || code.includes(q) || brand.includes(q) || group.includes(q) || sub.includes(q);
    });
  }

  // Grup -> Alt Grup -> ürünler
  // Map(group => Map(sub => []))
  const tree = new Map();
  list.forEach(p => {
    const g = (p.group || "DİĞER").trim() || "DİĞER";
    const s = (p.subcategory || "DİĞER").trim() || "DİĞER";
    if (!tree.has(g)) tree.set(g, new Map());
    const subMap = tree.get(g);
    if (!subMap.has(s)) subMap.set(s, []);
    subMap.get(s).push(p);
  });

  const groups = Array.from(tree.keys()).sort((a,b)=>a.localeCompare(b,"tr"));
  ...
  let html = `<div class="print-title">${escapeHtml(titleText)}</div>`;

  groups.forEach(g => {
    html += `<div class="print-group">${escapeHtml(g)}</div>`;

    const subMap = tree.get(g);
    const subs = Array.from(subMap.keys()).sort((a,b)=>a.localeCompare(b,"tr"));

    subs.forEach(s => {
      const items = subMap.get(s) || [];
      // Ürünleri istersen koda göre sırala:
      items.sort((x,y) => (x.code||"").localeCompare((y.code||""), "tr"));

      html += `<div class="print-sub">${escapeHtml(s)}</div>`;
          ...
          <div class="print-card">
            ...
            <div>
              ...
              <div class="print-meta">
                <div><strong>Kod:</strong> ${escapeHtml(p.code || "")}</div>
                <div><strong>Marka:</strong> ${escapeHtml(p.brand || "")}</div>
              </div>
            </div>
          </div>
        `;
...
// Ctrl+P öncesi otomatik oluştur
window.addEventListener("beforeprint", buildPrintArea);
// Print bitince temizle (istersen kalsın diye bunu kaldırabilirsin)
window.addEventListener("afterprint", clearPrintArea);

// Bazı tarayıcılarda daha stabil:
if (window.matchMedia) {
  const mq = window.matchMedia("print");
  mq.addEventListener?.("change", (e) => {
    if (e.matches) buildPrintArea();
    else clearPrintArea();
  });
}